...
class ADMMDeepUnfolding(nn.Module):
    def __init__(self, N, D, EP, lam):
        ...
        self.gamma_raw = nn.Parameter(torch.tensor(1.0))  # 初期値 log(exp(10)-1)
        #self.lam_raw = nn.Parameter(torch.tensor(0.1))  # 初期値 0.1
    ...
    def forward(self, y, x_0):
       ...
       # γをsoftplusで得たあと制限(γに制約をつけて暴走し過ぎないようにしてみるコード)
       #gamma = torch.clamp(self.softplus(self.gamma_raw), min=0.1, max=20.0)

       #lam = self.softplus(self.lam_raw)  # 正の λ を取得(λも同時に学習できるコード)
       ...
       for _ in range(self.EP):
        ...
        #mseだけでなくPSNRのような損失関数を評価に取り入れる
        alpha = 10  # PSNR-like term の重み
        mse = torch.mean((x - x_0) ** 2)
        psnr_like = -10 * torch.log10(mse + 1e-8)
        loss = torch.relu(mse + alpha * psnr_like) #負にならないように
        losses.append(loss)

        #損失がマイナスになるのを防ぐ&PSNRの比重高く
        mse = torch.mean((x - x_0) ** 2)
        psnr_like = 10 * torch.log10(1.0 / (mse + 1e-8))
        alpha = 10.0
        psnr_offset = 40.0  # 上限PSNR
        loss = mse - alpha * psnr_like + alpha * psnr_offset
        losses.append(loss)
        '''''
       ...
       return x, mse  #最後のlossを見る
    
    def get_gamma(self):
        return self.softplus(self.gamma_raw).item()
    #def get_lambda(self):
        #return self.softplus(self.lam_raw).item()
...
for epoch in range(epochs):
    ...
    #print(f"Epoch {epoch+1}: Loss = {total_loss / num_blocks:.6f}, γ = {model.get_gamma():.6f}, λ = {model.get_lambda():.6f}")
GAM = model.get_gamma()
